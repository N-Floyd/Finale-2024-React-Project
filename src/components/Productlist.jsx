...
const Productlist = () => {
    ...
    const showProducts = showLatest ? productlist.slice(8, 12) : productlist.slice(4, 8);
            ...
            <div>
                <div className="flex justify-center gap-[24px]">
                    <button onClick={() => handleClick('featured')} 
                        className={`w-[92px] h-[31px] font-semibold border-solid border-2 rounded-[100px] ${!showLatest ? 'border-[#E9E9EB]' : ''} text-[14px] hover:scale-110 hover:bg-gray-200 duration-500`} >Featured
                    </button>
                    <button onClick={() => handleClick('latest')} 
                        className={`text-[14px] ${showLatest ? 'text-[#5C5F6A]' : 'text-gray-400'} hover:scale-110 duration-500`} >Latest
                    </button>
                </div>
